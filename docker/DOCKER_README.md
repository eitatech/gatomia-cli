# GatomIA Docker Setup

This document explains how to run GatomIA using Docker and Docker Compose.

## Overview

The Docker setup provides a containerized environment for running the GatomIA web application, which allows you to generate documentation for GitHub repositories through a web interface.
...
LOGFIRE_TOKEN=
LOGFIRE_PROJECT_NAME=gatomia
LOGFIRE_SERVICE_NAME=gatomia
```

### 3. Create Docker Network

```bash
docker network create gatomia-network
```
...
The `docker-compose.yml` file defines the GatomIA service with the following features:

### Service Configuration

- **Image**: `gatomia:0.0.1`
- **Build Context**: Parent directory (`.` relative to docker/)
- **Container Name**: `gatomia`
- **Port Mapping**: `${APP_PORT:-8000}:8000`
- **Network**: `gatomia-network` (external)
...
The Dockerfile (`docker/Dockerfile`) builds the GatomIA image with:
...
- **Exposed Port**: `8000`
- **Entry Point**: `python gatomia/run_web_app.py --host 0.0.0.0 --port 8000`
...
```bash
docker exec -it gatomia /bin/bash
```
...
- **Network not found**: Create network with `docker network create gatomia-network`
...
docker inspect gatomia --format='{{.State.Health.Status}}'

# View health check logs
docker inspect gatomia --format='{{range .State.Health.Log}}{{.Output}}{{end}}'
```
...
docker-compose -f docker/docker-compose.yml down
# Add to docker-compose.yml under 'gatomia' service:
# user: "${UID}:${GID}"
```
...
3. Add GitHub to known_hosts:
   ```bash
   docker exec -it gatomia ssh-keyscan github.com >> /root/.ssh/known_hosts
   ```
...
1. Check logs: `docker logs gatomia`
2. Verify configuration: `docker exec gatomia env | grep -E '(LLM|APP)'`
3. Test connectivity: `docker exec gatomia curl -I http://localhost:8000`
4. Report issues: https://github.com/yourusername/gatomia/issues
