...
class GatomIAError(Exception):
    """Base exception for GatomIA CLI errors."""
...
class ConfigurationError(GatomIAError):
...
class RepositoryError(GatomIAError):
...
class FileSystemError(GatomIAError):
...
def handle_error(error: Exception, verbose: bool = False) -> int:
    ...
    """
    if isinstance(error, GatomIAError):
