...
from gatomia.cli.utils.instructions import display_post_generation_instructions
from gatomia.cli.models.job import GenerationOptions
...
@click.option(
    ...
    type=str,
    default=None,
    help="Fallback model to use (overrides config)",
)
@click.option(
    "--max-tokens",
    type=int,
...
def generate_command(
    ...
    $ gatomia generate --create-branch --github-pages

    \b
    # Force full regeneration
    $ gatomia generate --no-cache

    \b
    # C# project: only .cs files, exclude tests
    $ gatomia generate --include "*.cs" --exclude "*Tests*,*Specs*"

    \b
    # Focus on specific modules with architecture docs
    $ gatomia generate --focus "src/core,src/api" --doc-type architecture

    \b
    # Custom instructions
    $ gatomia generate --instructions "Focus on public APIs and include usage examples"

    \b
    # Override max tokens for this generation
    $ gatomia generate --max-tokens 16384

    \b
    # Set all max token limits
    $ gatomia generate --max-tokens 32768 --max-token-per-module 40000 --max-token-per-leaf-module 20000

    \b
    # Override max depth for hierarchical decomposition
    $ gatomia generate --max-depth 3
    """
    ...
    try:
        ...
        if not config_manager.load():
            raise ConfigurationError(
                ...
                "Please run 'gatomia config set' to configure your LLM API credentials:\n"
                "  gatomia config set --api-key <your-api-key> --base-url <api-url> \\\n"
                "    --main-model <model> --cluster-model <model>\n\n"
                "For more help: gatomia config --help"
            )
        ...
        if output_dir.exists() and list(output_dir.glob("*.md")):
            ...
            msg += "Overwrite (Force Regeneration)?" if no_cache else "Update/Resume?"
