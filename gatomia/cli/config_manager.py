...
from gatomia.cli.models.config import Configuration
from gatomia.cli.utils.errors import ConfigurationError, FileSystemError
from gatomia.cli.utils.fs import ensure_directory, safe_write, safe_read


# Keyring configuration
KEYRING_SERVICE = "gatomia"
KEYRING_API_KEY_ACCOUNT = "api_key"

# Configuration file location
CONFIG_DIR = Path.home() / ".gatomia"
...
class ConfigManager:
    """
    Manages GatomIA configuration with secure keyring storage for API keys.

    Storage:
        - API key: System keychain via keyring (macOS Keychain, Windows Credential Manager,
                  Linux Secret Service)
        - Other settings: ~/.gatomia/config.json
    """
    ...
    def save(
        ...
        Args:
            ...
            llm_provider: LLM Provider (openai or copilot)
            copilot_token: GitHub Copilot Token (optional)
        ...
        if copilot_token is not None:
            self._config.copilot_token = copilot_token
        ...
        if include_reasoning is not None:
            self._config.include_reasoning = include_reasoning
    ...
    def is_configured(self) -> bool:
        ...
        # Check if API key is set (only required for non-copilot providers)
        if self._config.llm_provider != "copilot" and self.get_api_key() is None:
            return False
