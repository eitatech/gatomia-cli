...
def get_leaf_nodes(graph: Dict[str, Set[str]], components: Dict[str, Node]) -> List[str]:
    ...
    def concise_node(leaf_nodes: Set[str]) -> Set[str]:
        ...
        for node in list(concise_leaf_nodes):
            if node in components:
                if components[node].component_type in valid_types:
                    keep_leaf_nodes.append(node)
            else:
                # If the concise name isn't found, try finding the original that produced it
                # Actually, concise_leaf_nodes is more of a set of *candidate* IDs.
                pass
